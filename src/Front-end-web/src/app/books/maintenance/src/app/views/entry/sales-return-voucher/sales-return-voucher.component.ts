...
export class SalesReturnVoucherComponent {
 ...
 constructor(
   ...
   onCostPriceChange() {
    const itemsFormArray = <FormArray>this.purchaseForm.get('multipleItemEntry');
    
    itemsFormArray.controls.forEach((control, index) => {
      control.get('costPrice').valueChanges.subscribe(newCostPrice => {
        // Update the respective item in listOfItem with the new costPrice
        this.listOfItem[index].costPrice = newCostPrice;
      });
    });
  }
   ...
   addItem(): FormGroup {
    return this._fb.group({
      ...
      itemSubLedgerCode: [''],
      itemLedgerCode: [''],
      costPrice: [0], // Add the costPrice field here
    });
  }
