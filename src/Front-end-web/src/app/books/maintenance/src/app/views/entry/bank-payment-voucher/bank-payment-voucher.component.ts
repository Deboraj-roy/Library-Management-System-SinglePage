...
export class BankPaymentVoucherComponent {
  ...
  getDebitAmount(amount:any){
    ...
    for(i=0;i<(<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).length;i++){
      this.debitAmounttotal += (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('amount').value;
    }
  }
  getCreaditAmount(amount:any){
    var i=0;
    this.creditAmounttotal=0;
    for(i=0;i<(<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).length;i++){
      this.creditAmounttotal+=(<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).at(i).get('amount').value;
    }
  }
 ...
 showaAvanceLedgerNgSelect(i: number): boolean {
  const val = (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('debitSubLedgerCode').value;
  ...
  getledgerTo(uniqid:any,i:any){
   if(uniqid!=null){
    ...
    if(this.receiptModel.subLedgerCode === "12001" || this.receiptModel.subLedgerCode === "12002"){
      (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('supplierCostingHeadId').setValue(this.getAllSupplierCostingHeads(this.receiptModel.subLedgerId));
      (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('supplierCostingHeadId').enable();
    }else{
      (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('supplierCostingHeadId').disable();
  (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('supplierCostingHeadId').patchValue(null);
    }
     ...
     (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('debitbalance').patchValue(this.fromamount);
     (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('fromName').patchValue(this.fromName);
    });
    (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).patchValue({
    ...
    (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('debitbalance').patchValue(null);
    (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).at(i).get('fromName').patchValue(null);
  ...
  getledgerFrom(uniqIdCredit:any,i:any){
      if(uniqIdCredit!=null){
        ...
        (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).at(i).get('creaditbalance').patchValue(this.toamount);
        (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).at(i).get('toName').patchValue(this.toName);
       });
      (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).at(i).patchValue({
        ...
        (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).at(i).get('creaditbalance').patchValue(null);
        (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).at(i).get('toName').patchValue(null);
      }
  }

  addDebit(){
    (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).push(this.addBankPaymentCredit());
  }
  removeDebit(i:any){
    (<FormArray>this.bankPaymentForm.get('multipleCreaditEntry')).removeAt(i);
  }

  removeCreadit(i:any){
    (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).removeAt(i);
  }
  addCreadit(){
    (<FormArray>this.bankPaymentForm.get('multipleDebitEntry')).push(this.addBankPaymentDebit());
  }
    ...
    this.bankPaymentForm=this._fb.group({
      narration:[,[Validators.required]],
        ...
        multipleDebitEntry:this._fb.array([
         this.addBankPaymentDebit()
        ]),
        multipleCreaditEntry:this._fb.array([
          this.addBankPaymentCredit()
         ]),
    })
  }

  addBankPaymentCredit():FormGroup{
  ...
  get fromval(){
    return this.bankPaymentForm.value;
  }
  get g() {
